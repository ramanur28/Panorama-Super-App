...
export const DriverJeepView: React.FC<Props> = ({ currentUser, onNotify, onRequestConfirm }) => {
  ...
  return (
    <div className="card-section">
      {/* Worker Identity Hero Card (Clean Light Styling) */}
      <div
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: '12px', color: '#475569' }}>
              Unit Armada: <strong style={{ color: '#0f172a' }}>{currentUser.vehicleUnit || 'Armada Jeep'}</strong>
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <span className={`badge ${currentUser.type === 'internal' ? 'badge-internal' : 'badge-external'}`}>
              Armada {currentUser.type.toUpperCase()}
            </span>
            <div style={{ fontSize: '11px', color: '#64748b', marginTop: '4px', fontWeight: 600 }}>
              Tarif: {formatIDR(currentUser.baseRatePerTrip)} / trip
            </div>
          </div>
        </div>
      </div>

      {/* Auto-status Live Info Badge */}
      <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', padding: '10px 14px', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: '#475569', boxShadow: '0 1px 2px rgba(0,0,0,0.02)' }}>
        ...
        <span>Status jalan diperbarui <strong>otomatis</strong> sesuai jam jadwal penjemputan.</span>
      </div>

      {/* Worker Work Frequency & Pay Metrics */}
      <div className="metrics-grid">
        <MetricCard
          title="Total Hari Kerja"
          value={`${payrollSummary?.totalDaysWorked || 0} Hari`}
          icon={<Calendar size={16} />}
          subtitle={`${payrollSummary?.totalTripsCount || 0} Kali Trip Dijalankan`}
          accentColor="#ea580c"
        />
        <MetricCard
          title="Total Bayaran Anda"
      ...
      <div style={{ display: 'flex', gap: '6px', background: '#f1f5f9', padding: '4px', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
        ...
        >
          <Clock size={13} />
          <span>Trip Hari Ini ({todayTrips.length})</span>
        </button>
        <button
          type="button"
      ...
      <div className="card-section">
        <div className="section-header">
          <div className="section-title">
            <span>{activeTab === 'today' ? 'Jadwal Penjemputan Hari Ini' : 'Daftar Seluruh Penugasan'}</span>
          </div>
          <span style={{ fontSize: '11px', color: '#64748b' }}>
            Ketuk kartu untuk rincian
          </span>
        </div>
        ...
        {displayTrips.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '36px 16px', color: '#64748b', background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '12px' }}>
            Belum ada jadwal trip yang ditugaskan ke Anda pada periode ini.
          </div>
        ) : (
          displayTrips.map((trip) => (
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
                <div>
                  <span style={{ fontSize: '11px', color: '#64748b' }}>Honor Anda: </span>
                  {trip.tripStatus === 'cancelled' ? (
                    ...
                    <>
                      ...
                      <span style={{ marginLeft: '6px' }} className={`badge ${trip.jeepPayrollStatus === 'paid' ? 'badge-paid' : 'badge-unpaid'}`}>
                        {trip.jeepPayrollStatus === 'paid' ? 'Cair' : 'Pending'}
                      </span>
                    </>
                  )}
                </div>
